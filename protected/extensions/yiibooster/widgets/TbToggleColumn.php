...
class TbToggleColumn extends TbDataColumn {
	...
	public function init() {
		if ($this->name === null) {
			throw new CException ( Yii::t ( 'zii', '"{attribute}" attribute cannot be empty.', array (
					'{attribute}' => "name" 
			) ) );
		}
	...
	protected function initButton() {
		...
		$this->button = array (
				...
				'htmlOptions' => array (
						'class' => $this->name . '_toggle' . $this->uniqueClassSuffix 
				) 
		);
...
function() {
	...
	protected function renderDataCellContent($row, $data) {
		$checked = ($this->value === null) ? CHtml::value ( $data, $this->name ) : $this->evaluateExpression ( $this->value, array (
				'data' => $data,
				'row' => $row 
		) );
