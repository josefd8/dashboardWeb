...
class TbExtendedGridView extends TbGridView {
	...
	public function renderChart() {
		...
		$buttons = Yii::createComponent ( array (
				...
				'buttons' => array (
						array (
								...
								'url' => '#',
								'htmlOptions' => array (
										'class' => 'active ' . $this->getId () . '-grid-control grid' 
								) 
						),
						array (
								...
								'url' => '#',
								'htmlOptions' => array (
										'class' => $this->getId () . '-grid-control chart' 
								) 
						) 
				),
				'htmlOptions' => array (
		...
		if (! empty ( $this->chartOptions ['data'] ['xAxis'] )) {
			...
			if (is_array ( $categories )) {
				...
				for($row = 0; $row < $count; ++ $row) {
					...
					if (! is_null ( $column ) && $column->value !== null) {
						$xAxisData ['categories'] [] = $this->evaluateExpression ( $column->value, array (
								'data' => $data [$row],
								'row' => $row 
						) );
					} else {
		...
		// ****************************************
		// render chart
		$options = CMap::mergeArray ( $this->chartOptions ['config'], array (
				'series' => $seriesData,
				'xAxis' => $xAxisData 
		) );
	...
	public function registerCustomClientScript() {
		...
		if ($this->sortableRows) {
			...
			if ($this->sortableAjaxSave && $this->sortableAction !== null) {
				$sortableAction = Yii::app ()->createUrl ( $this->sortableAction, array (
	...
	protected function getSummaryOperationInstance($name, $config) {
		...
		if (! in_array ( $config ['class'], $this->extendedSummaryOperations )) {
			throw new CException ( Yii::t ( 'zii', '"{operation}" is an unsupported class operation.', array (
...
abstract class TbOperation extends CWidget {
	...
	public function init() {
		if (null == $this->column) {
			throw new CException ( Yii::t ( 'zii', '"{attribute}" attribute must be defined', array (
					'{attribute}' => 'column' 
			) ) );
		}
	}
...
class TbPercentOfTypeEasyPieOperation extends TbPercentOfTypeOperation {
	 ...
	 */
	public function displaySummary() {
		$this->typeTemplate = strtr ( $this->typeTemplate, array (
