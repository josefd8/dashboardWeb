...
class TbActiveForm extends CActiveForm {
	...
	/**
	 * The form type.
	 * Allowed types are in `TYPE_*` constants.
	 * 
	 * @var string
	 */
	...
	/**
	 * Whether to render field error after input.
	 * Only for vertical and horizontal types.
	 * 
	 * @var bool
	 */
	 ...
	 * @param array $widgetOptions
	 *        	List of initial property values for the CCaptcha widget (Property Name => Property Value).
	 * @param array $options
	 *        	Group attributes.
	 * @return string The generated color picker group.
	 * @see CCaptcha
	 ...
	 *        	Pre-rendered field as string or array of arguments for call_user_func_array()
	 *        	function.
	 * @param CModel $model
	 *        	The data model.
	 * @param string $attribute
	...
	protected function setDefaultPlaceholder(&$fieldData) {
		if (! is_array ( $fieldData ) || empty ( $fieldData [0] [1] ) /* 'textField' */
	...
	protected function renderAddOnBegin($prependText, $appendText, $prependOptions) {
		...
		echo CHtml::tag ( $this->addOnWrapperTag, array (
				'class' => implode ( ' ', $wrapperCssClass ) 
		), false, false );
		if (! empty ( $prependText )) {
