...
class TbHtml extends CHtml // required in order to access the protected methods in CHtml
{
	...
	public static function statefulFormTb($layout = self::FORM_LAYOUT_VERTICAL, $action = '', $method = 'post', $htmlOptions = array()) {
		return self::formTb ( $layout, $action, $method, $htmlOptions ) . self::tag ( 'div', array (
				'style' => 'display:none' 
...
EOD;
	...
	public static function activeRadioButtonList($model, $attribute, $data, $htmlOptions = array()) {
		...
		$hiddenOptions = isset ( $htmlOptions ['id'] ) ? array (
				'id' => CHtml::ID_PREFIX . $htmlOptions ['id'] 
		) : array (
				'id' => false 
		);
...
self::tag ( 'span', $addOnOptions, $addOn ) : $addOn;
	...
	public static function breadcrumbs($links, $htmlOptions = array()) {
		...
		foreach ( $links as $label => $url ) {
			if (is_string ( $label )) {
				...
				echo self::tag ( 'span', array (
						'class' => 'divider' 
				), $divider );
				echo '</li>';
			} else
				echo self::tag ( 'li', array (
						'class' => 'active' 
				), $url );
		}
		echo '</ul>';
	 ...
	 *        	the class(es) to append to `$htmlOptions`
	 * @param array $htmlOptions
	 *        	the HTML tag attributes to modify
	 ...
	 * 
	 * @param array $names
	 *        	the option names to move.
	 * @param array $fromOptions
	 *        	the options to move from.
	 * @param array $options
	 *        	the options to move to.
	 * @return array the options.
	 */
	 ...
	 * 
	 * @param array $options
	 *        	the options to set defaults for.
	 ...
	 *        	the name of the item.
	 * @param array $options
	 *        	the options to remove the item from.
	 * @param mixed $defaultValue
	 *        	the default value.
	 * @return mixed the value.
	 */
	 ...
	 * 
	 * @param array $options
	 *        	the options to remove from.
	 * @param array $names
	 *        	names to remove from the options.
	 * @return array the options.
	 */
